...
def render_settings(screen, mx, my, m_c, m_down, key_escape, tap_snd, ui_pulse_t, menu_bg,
    ...
    btn_mob = pygame.Rect(440, 105, 270, 82)
    ...
    # ── Visual Quality ──────────────────────────────────────
    draw_text("VISUAL QUALITY", FONT_TINY, (120, 150, 190), 90, 218, center=False)

    quality_btns = [
        ("PERFORMANCE", 'low',    NEON_PINK,   pygame.Rect(90,  232, 152, 36)),
        ("BALANCED",    'medium', NEON_GOLD,   pygame.Rect(254, 232, 152, 36)),
        ("CINEMATIC",   'high',   NEON_CYAN,   pygame.Rect(418, 232, 152, 36)),
    ]
    ...
    draw_divider(screen, 90, 282, 710, NEON_CYAN, alpha=30)

    # ── iOS-style Toggle Switches ────────────────────────────
    draw_text("GAMEPLAY OPTIONS", FONT_TINY, (120, 150, 190), 90, 298, center=False)

    toggles = [
        ("DAMAGE NUMBERS",  show_damage_enabled,  NEON_GOLD,   90,  328),
        ("AUTO-FIRE",       auto_fire_enabled,     NEON_GREEN,  340, 328),
        ("SCREEN SHAKE",    screen_shake_enabled,  NEON_PINK,   90,  374),
        ("SHOW FPS",        show_fps,              NEON_CYAN,   340, 374),
    ]
    ...
    draw_divider(screen, 90, 422, 710, NEON_CYAN, alpha=30)

    # ── Neon Volume Sliders ──────────────────────────────────
    draw_text("AUDIO", FONT_TINY, (120, 150, 190), 90, 440, center=False)

    music_vol, _ = _draw_neon_slider(screen, 90,  462, 240, music_vol,
                                     "MUSIC VOLUME", accent=NEON_TEAL, mx=mx, my=my, m_down=m_down)
    sfx_vol, _   = _draw_neon_slider(screen, 400, 462, 240, sfx_vol,
                                     "SFX VOLUME",   accent=NEON_AMBER, mx=mx, my=my, m_down=m_down)

    if m_down:
        pygame.mixer.music.set_volume(music_vol)

    draw_divider(screen, 90, 494, 710, NEON_CYAN, alpha=30)

    # ── Account & System Buttons ─────────────────────────────
    btn_bind = pygame.Rect(90, 510, 230, 44)
    ...
    btn_win = pygame.Rect(340, 510, 200, 44)
    is_h_win = btn_win.collidepoint(mx, my)
    draw_plasma_button(screen, "PC CONTROLS", FONT_SMALL, WHITE, btn_win,
    ...
    btn_back = pygame.Rect(554, 510, 156, 44)
